...
`cmake/check_dtcm_residency.py` and `cmake/check_psram_ai_residency.py` run beside it
and are the same idea applied to data. Each guards a rule with **no runtime
enforcement**, in both directions, and each exists because the corresponding mistake is
silent rather than loud:

- a DMA buffer that ends up in **DTCM** moves no data at all — deterministically, every
  transfer, with no fault (issue #46);
- a DMA buffer that ends up in the **cacheable PSRAM carve-out** is worse. It transfers
  correctly and is then read back stale from the data cache, so it depends on cache
  pressure and can pass every test on an idle board (issue #9). That check therefore
  tests against the **MPU region**, not the linker section: the region is what makes
  memory cacheable, and it covers its whole 2 MB whether or not the linker put anything
  there.

The third check in the PSRAM guard is worth naming separately: the carve-out's base
