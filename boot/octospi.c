/*
 * OCTOSPI2 bring-up + erase/program for the standalone bootloader (W25Q128).
 *
 * The bootloader runs from internal flash (not XIP from OCTOSPI2), so these are
 * ordinary flash-resident functions: no code executes from, and nothing
 * speculatively reads, 0x70000000 while OCTOSPI2 is out of memory-mapped mode,
 * so no RAM-resident driver or cache juggling is needed.  Caches stay OFF
 * throughout the bootloader, which keeps the register / mmap accesses coherent.
 *
 * Reproduces TinyUF2's captured config (see boot/README.md):
 *   - OCTOSPIM stays at reset (OCTOSPI2 -> Port 2); GPIO banks F & G replayed
 *     verbatim (they carry the Port-2 flash pins).
 ...
 * Ensures the W25Q Quad-Enable (QE, SR2 bit 1) is set before quad reads.
 *
 * W25Q128 1-line commands (independent of the mmap quad config): WREN 0x06,
 * RDSR1 0x05 (WIP=bit0), RDSR2 0x35, WRSR2 0x31, 4K sector erase 0x20, page
 * program 0x02, JEDEC id 0x9F.  24-bit addressing.
 */
...
#define CCR_ADSIZE_24 (2u << OCTOSPI_CCR_ADSIZE_Pos)   // 24-bit addr (W25Q128)
...
// Memory-mapped 0xEB Quad I/O read config (from the dump); re-applied to
// re-enter mmap after an erase/program leaves it for indirect commands.
...
static void write_reg(uint8_t instr, const uint8_t *buf, uint32_t n)
{
  ...
  {
    wait_flag(OCTOSPI_SR_FTF);
    *(volatile uint8_t *)&OCTOSPI2->DR = buf[i];
  }
  wait_flag(OCTOSPI_SR_TCF);
  OCTOSPI2->FCR = OCTOSPI_FCR_CTCF;
}
...
// Ensure W25Q Quad-Enable (SR2 bit 1).  Non-volatile, so usually already set;
// only program it if missing (status-register endurance).
...
// MODER written last so the alternate function is selected before it activates.
static void gpio_replay(GPIO_TypeDef *g, uint32_t moder, uint32_t otyper,
                        uint32_t ospeedr, uint32_t pupdr,
...
// Bring up OCTOSPI2 memory-mapped mode; returns 1 if the flash JEDEC id looks
// like a Winbond W25Q (mfr 0xEF).  jedec[3] receives the id.
int octospi2_init(uint8_t jedec[3])
{
  __HAL_RCC_GPIOF_CLK_ENABLE();
  __HAL_RCC_GPIOG_CLK_ENABLE();
  __HAL_RCC_OSPI2_CLK_ENABLE();
  // OCTOSPI1 is clocked too (captured AHB3ENR has OSPI1EN set): the bank replay
  // below routes the PSRAM's Port-1 pins (PF6-10, PG6) to OCTOSPI1 in AF mode,
  // and an AF pin should not hang off an unclocked peripheral.  Idle OCTOSPI1
  // keeps its NCS high, so the PSRAM (external pull-up too) stays deselected --
  // the same state TinyUF2 left.
  __HAL_RCC_OSPI1_CLK_ENABLE();
  __HAL_RCC_OCTOSPIM_CLK_ENABLE();

  // Replay TinyUF2's exact GPIO for the OCTOSPI banks.  This covers the W25Q128
  // app flash on OCTOSPIM Port 2 -- PF4=CLK(AF9), PG12=NCS(AF3), PG0/PG1/PG10/
  // PG11=IO4-7 (AF9/AF3, quad data on the P2 high nibble) per schematic sheet 6
  // ("QSPI2_*" nets) -- and the PSRAM's Port-1 pins (PF6-10, PG6, "OSPI1_*"
  // nets).  Net names do NOT match peripheral numbers; replaying both banks
  // verbatim sidesteps that trap.
  gpio_replay(GPIOF, 0xFFEAAEF3u, 0x00000000u, 0x003FF300u,
              0x00000004u, 0xAA090000u, 0x000009AAu);
  gpio_replay(GPIOG, 0xFEAFEFFAu, 0x00000000u, 0x03F0300Fu,
              0x00000000u, 0x0A000099u, 0x00039300u);
...
//--------------------------------------------------------------------+
// Erase / program.  Leave mmap (abort -> indirect), issue the W25Q command,
// then restore mmap.  Runs from internal flash with caches off, so the mmap-off
// window is safe.
//--------------------------------------------------------------------+

// Leave memory-mapped mode for indirect commands (RM0468: abort then reconfig).
static void abort_to_indirect(void)
{
  __DSB();
  OCTOSPI2->CR |= OCTOSPI_CR_ABORT;
  { uint32_t n = SPIN; while ((OCTOSPI2->CR & OCTOSPI_CR_ABORT) && n) n--; }
  wait_not_busy();
  OCTOSPI2->FCR = OCTOSPI_FCR_CTCF | OCTOSPI_FCR_CTEF;
...
static void program_page(uint32_t addr, const uint8_t *data, uint32_t n)
{
  ...
  OCTOSPI2->IR  = 0x02u;
  OCTOSPI2->AR  = addr;                // triggers; data pulled via the FIFO
  for (uint32_t i = 0; i < n; i++)
  {
    wait_flag(OCTOSPI_SR_FTF);
...
// Erase the 4 KB sector containing `addr` (flash offset), restore mmap.
...
// Program `len` bytes at `addr` (flash offset), splitting on 256 B pages, then
// restore mmap.  Caller erases the covering sector(s) first.
