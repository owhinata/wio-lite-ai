...
2. **Phase 2（重要・大仕事）**: 実ブートローダ@`0x08000000`化。**app-first期はクロック/OCTOSPI2 mmap設定をTinyUF2から継承していたが、置換後は全部自前でやる必要**：
   ...
   - **bootrom全milestone(1-4)完了+レビュー済。次=Phase 3(実機書込&検証)。** 注意: milestone4のjump/ボタン/manifest-rebootは全てuntested(Phase 3で要検証)。特にmanifest後の自動reboot挙動はdfu-util実挙動で要確認(必要なら`-R`/手動resetにfallback、または遅延調整)。
